# D:\CodexWorkspace 的 Codex 执行规则

本文件是 `D:\CodexWorkspace` 的 Codex 执行规则文件。Codex 进入本工作区后，必须优先遵守本文件；`codexworkspace-console.md` 是总控索引和说明书。
...
- 所有 Codex 工作默认只在 `D:\CodexWorkspace` 内完成。
- 全局 Codex 规则建议：无论以后打开哪个 Codex 项目，都必须先迁移到 `D:\CodexWorkspace` 再继续工作。
- 不得在 `D:\CodexWorkspace` 之外直接创建、编辑、运行或整理项目文件。
- 如任务涉及外部项目，必须先说明外部路径、迁移目的和影响，并获得用户明确同意。
- 外部项目获得同意后，先迁移或复制到 `D:\CodexWorkspace\projects\Project_编号_项目名称`，后续只操作工作区内副本。
- 除非用户明确批准，不维护或修改 `D:\CodexWorkspace` 之外的清单、日志或项目文件。
