...
本文件是 `D:\CodexWorkspace` 的新项目与外部项目迁移流程 SOP。
...
3. 创建项目目录
   - 默认路径格式：`D:\CodexWorkspace\projects\Project_三位编号_项目名称`。
   - 项目名称应清晰表达用途。
   - 不直接在根目录创建正式项目。
...
外部项目必须先迁移到 `D:\CodexWorkspace` 后再继续工作。
...
4. 复制到工作区
   - 迁移目标路径：`D:\CodexWorkspace\projects\Project_三位编号_项目名称`。
   - 默认复制外部项目，不直接移动原项目。
   - 迁移完成后，只操作工作区内副本。
...
- 项目是否位于 `D:\CodexWorkspace\projects\Project_三位编号_项目名称`。
